...
from app.shared.query_helpers import get_user_children, extract_id_from_link
from app.modules.parent.application.onboarding_service import _calculate_fallback_traits
...
def _calculate_age_years(birth_date: datetime) -> int:
    now = datetime.now(birth_date.tzinfo) if birth_date.tzinfo else datetime.now()
    return max(0, (now - birth_date).days // 365)
...
async def create_child(
    ...
    age = _calculate_age_years(child.birth_date)
    ...
    from app.modules.ai.application.generation_service import generate_initial_tasks_for_child
...
async def get_children(current_user: User = None):
    """
...
async def delete_child(
    ...
    from app.modules.rewards.domain.models import ChildReward
    from app.modules.children.domain.models import ChildDevelopmentAssessment
