...
from app.modules.tasks.domain.models import ChildTask, ChildTaskStatus, ChildTaskPriority
...
async def analyze_emotion_report_and_generate_tasks(
    ...
    try:
        ...
        if request.report_id:
            ...
            if not child_reports:
                ...
                try:
                    ...
                    from app.modules.reports.application.report_service import _generate_report_internal
        ...
        # Calculate age
        age = utc_now().year - child.birth_date.year
        if utc_now().month < child.birth_date.month or (
            utc_now().month == child.birth_date.month and 
            utc_now().day < child.birth_date.day
        ):
            age -= 1
        ...
        if inserted_count > 0 and len(created_tasks) == 0:
            ...
            try:
                ...
                recent_unassigned = [
                    ...
                    and ct.assigned_at and (utc_now() - ct.assigned_at).total_seconds() < 120
                ]
        ...
        try:
            ...
            try:
                ...
                recent_unassigned = [
                    ct for ct in all_child_tasks 
                    if ct.status == ChildTaskStatus.UNASSIGNED 
                    and ct.assigned_at and (utc_now() - ct.assigned_at).total_seconds() < 60
                ]
                recent_unassigned.sort(key=lambda x: x.assigned_at, reverse=True)
...
async def calculate_skills_from_task_history(child: Child, child_tasks: List[ChildTask]) -> Dict[str, int]:
    ...
    for ct in child_tasks:
        ...
        if ct.status == ChildTaskStatus.COMPLETED:
            ...
            if ct.completed_at:
                days_ago = (utc_now() - ct.completed_at).days
                if days_ago <= 7:
