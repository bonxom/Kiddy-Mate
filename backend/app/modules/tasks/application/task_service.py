...
from app.modules.tasks.domain.models import Task, TaskCategory, TaskType
from app.modules.children.domain.models import Child
from app.modules.tasks.domain.models import ChildTask, ChildTaskStatus, ChildTaskPriority, TaskData
from app.schemas.schemas import TaskPublic, TaskCreate, ChildTaskPublic, ChildTaskWithDetails
from typing import List, Optional
from datetime import datetime, time
from app.core.time import utc_now
from app.shared.query_helpers import get_child_tasks_by_child, extract_id_from_link, fetch_link_or_get_object, ensure_link_references_for_save
from app.modules.rewards.domain.models import ChildReward, Reward
...
async def get_child_tasks(
    ...
    status_filter: Optional[ChildTaskStatus] = None
):
    """
...
async def start_task(
    ...
    If the task already exists for the child, transition it to `in_progress`
    when the current state allows it instead of creating a duplicate record.
    """
    try:
    ...
    if not task:
        raise HTTPException(
            ...
            detail="Task not found. Use /parent/children/{child_id}/tasks/suggested or /parent/tasks to get a valid task id."
        )
    ...
    if existing:
        ...
        if isinstance(current_status, str):
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Invalid task status: {existing.status}",
                )

        if current_status in {ChildTaskStatus.UNASSIGNED, ChildTaskStatus.ASSIGNED, ChildTaskStatus.GIVEUP}:
            update_data = {
                "status": ChildTaskStatus.IN_PROGRESS,
                "assigned_at": utc_now(),
            }
            if request.due_date is not None:
                update_data["due_date"] = parse_date_string(request.due_date)
            if request.priority is not None:
                update_data["priority"] = request.priority
            if request.notes is not None:
                update_data["notes"] = request.notes
            await existing.set(update_data)
            existing = await ChildTask.get(existing.id)
        elif current_status == ChildTaskStatus.IN_PROGRESS:
            pass
        elif current_status == ChildTaskStatus.NEED_VERIFY:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Task is already completed and waiting for parent verification.",
            )
        elif current_status == ChildTaskStatus.COMPLETED:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Task has already been completed and verified.",
            )
        elif current_status == ChildTaskStatus.MISSED:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Task has been missed and cannot be started again.",
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Task cannot be started from status '{current_status.value}'.",
            )
...
async def verify_task(
    ...
    from app.modules.dashboard.application.dashboard_service import update_child_skills
...
async def giveup_task(
    ...
    if not child_task:
        ...
        matching_tasks.sort(
            key=lambda ct: status_priority.get(ct.status, 99)  # type: ignore[arg-type]
        )
        ...
        if not child_task:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Task '{task_id}' is not assigned to this child. Please assign the task first using POST /parent/children/{child_id}/tasks/{task_id}/start"
            )
    ...
    if child_task.status not in [ChildTaskStatus.UNASSIGNED, ChildTaskStatus.ASSIGNED, ChildTaskStatus.IN_PROGRESS]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot give up task with status '{child_task.status.value}'. Task must be unassigned, assigned, or in progress."
        )
    
    if child_task.status in [ChildTaskStatus.UNASSIGNED, ChildTaskStatus.ASSIGNED]:
        await child_task.set({"status": ChildTaskStatus.IN_PROGRESS})
